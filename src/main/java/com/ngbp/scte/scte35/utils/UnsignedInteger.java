...
//	  public static final UnsignedInteger MAX_VALUE = fromIntBits(-1);
	  ...
	  private int value;
	  private int size; //8, 16, 32

	  private UnsignedInteger(int value, int size) {
		if(size != 8 && size != 16 && size != 32)  {
			throw new ArithmeticException("UnsignedInteger size must be 8, 16, or 32");
		}
	    this.size = size;
	    
	    //hack
	    if(size == 8) {
	    	this.INT_MASK = 0x000000FF;
	    } else if(size == 16) {
	    	this.INT_MASK = 0x0000FFFF;
	    } else if(size == 32) {
	    	this.INT_MASK = 0xFFFFFFFF;
	    } 

	    this.value = value & INT_MASK; //0xffffffff;
	  }
	   ...
	   * @since 14.0
	   */
	  public static UnsignedInteger fromIntBits(int bits, int size) {
	    return new UnsignedInteger(bits, size);
	  }
...
//	  }
	  ...
	  public String toString(int x, int radix) {
